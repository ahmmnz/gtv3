...
fun App() {
    ...
    var log by remember { mutableStateOf("Hazır - v2.1") }
    MaterialTheme {
        Column(Modifier.fillMaxSize().background(Color(0xFF0A0A0F)).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            Text("GTV Kumanda v2.1 HATASIZ", color=Color.White, fontSize=20.sp, fontWeight=FontWeight.Bold)
            ...
            Card(Modifier.fillMaxWidth(), colors=CardDefaults.cardColors(Color(0xFF1A1A2E))) {
                Column(Modifier.padding(12.dp)) {
                    ...
                    Row(horizontalArrangement=Arrangement.spacedBy(8.dp)) {
                        Button(onClick={ if(vol<100) vol++ }){ Text("Ses +") }
                        Button(onClick={ if(vol>0) vol-- }){ Text("Ses -") }
                    }
            ...
            Card(Modifier.fillMaxWidth(), colors=CardDefaults.cardColors(Color(0xFF1A1A2E))) {
                Column(Modifier.padding(12.dp)) {
                    ...
                    Row(horizontalArrangement=Arrangement.spacedBy(8.dp)) {
                        Button(onClick={ ch++ }){ Text("Kanal +") }
                        Button(onClick={ if(ch>1) ch-- }){ Text("Kanal -") }
                    }
                    ...
                    for(r in 0..3){
                        Row(Modifier.fillMaxWidth(), horizontalArrangement=Arrangement.spacedBy(4.dp)){
                            for(c in 0..2){
                                ...
                                else if(r==3 && c==1){ Button(Modifier.weight(2f), onClick={ typed.toIntOrNull()?.let{ch=it; log="Kanal $it"; typed=""} }){ Text("Git") } }
                            }
                        }
                    }
                }
            ...
            Text(log, color=Color(0xFF4ADE80))
        }
    }
}
